...
data Line a where
	...
	-- 2. the error message
	-- 3 and 4. the command sequence that was being processed when things died
	-- 5 and 6. the command sequence in the source file that was being expanded when things died
	-- 7. detailed message
	-- TODO: Error probably doesn't appear in the autogenerated code below
	Error          :: String -> String -> String -> String -> String -> String -> String -> Line a
...
--
-- 2. Two broken command sequences, one with what's currently being processed and the other with where we are in the source file.
--        \foo\bar
--                \baz
--        l.<number> \quux\bedazzle
--                                 \ricket\flam
--
-- 3. Detailed help. This is arbitrary text terminated by a blank line.
parseTeXError l s ss = fromMaybe giveUp (parseLaTeXError <|> parsePackageError <|> parseTeXError) where
	...
	parseTeXError = do
		(messageEnd, seqCurrBegin, seqCurrEnd, l', seqSourceBegin, seqSourceEnd, ss')
		                <- findCommandSequences ss
		(details, ss'') <- findBlankWithin 20 ss'
		return $ done (Error
		               	"TeX"
		               	(unwords (s:messageEnd))
		               	seqCurrBegin seqCurrEnd
		               	seqSourceBegin seqSourceEnd
		               	(unwords details)
		              ) (Just l') ss''
	...
	parseLaTeXOrPackageRemainder package messageBegin = do
		...
		(docInstr, seqCurrBegin, seqCurrEnd, l', seqSourceBegin, seqSourceEnd, ss'')
		                  <- findCommandSequences ss'
		(details, ss''')  <- findBlankWithin 20 ss''
		return $ done (Error
		               	package
		               	(unwords (messageBegin:messageEnd))
		               	seqCurrBegin seqCurrEnd
		               	seqSourceBegin seqSourceEnd
		               	(unwords details)
		              ) (Just l') ss'''
	...
	findCommandSequences ss = do
		...
		lineMark <- stripPrefix "l." lineHerald
		(l, seqSourceBegin) <- case reads lineMark of
			(n,s):_ -> return (n,s)
			_ -> Nothing
		return (b, trim seqCurrBegin, trim seqCurrEnd, LineMarker l, trim seqSourceBegin, trim seqSourceEnd, e)
...
prettyPrint = concatMap (go []) where
	...
	pprintMess (Error p s beg end beg' end' _) = p ++ " error: " ++ s ++ "\n\t" ++ beg' ++ "\n\t" ++ end' ++ "\n\t" ++ beg ++ "\n\t" ++ end
